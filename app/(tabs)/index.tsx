...
const HomeScreen = () => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      const fetchCounts = async () => {
        ...
        try {
          // 学習状況の件数を取得
          const { data: studyStatusData, error: studyStatusError } =
            await supabase.functions.invoke("count-study-status", {
              method: "POST",
              headers: {
          ...
          // 学習する単語の件数を取得
          const { data: studyCountData, error: studyCountError } =
            await supabase.functions.invoke("get-study-count", {
              method: "POST",
              headers: {
                Authorization: `Bearer ${session.access_token}`,
              },
  ...
  // 学習状態ごとに単語帳画面に遷移する関数
  const navigateToVocabularyWithFilter = (
    studyStatus: "学習済み" | "学習中"
  ) => {
    router.push({
      pathname: "/vocabulary",
      params: { studyStatus },
